...
class ComputeBoP(om.ExplicitComponent):
    ...
    BoP usually consists of :
        - Fuel cell stack and electrochemical system
        - Air delivery and water recovery system
    ...
    The last two subsystems (HEX and battery overhead) along with the compressor are computed outside of this discipline
    which is why a fitting parameter has been added in input to adjust the output dimensions.
    ...
    def setup(self):
        ...
        self.add_input("data:geometry:hybrid_powertrain:bop:pressure_reg_volume", val=np.nan, units='L')
        self.add_input("data:geometry:hybrid_powertrain:bop:fitting_factor", val=1, units='L',
                       desc='Scaling factor to account for the subsystems computed outside of this discipline')

        # self.add_output("data:geometry:hybrid_powertrain:bop:cooling_ss_volume", units='L')
        self.add_output("data:geometry:hybrid_powertrain:bop:fc_ss_volume", units='L')
        self.add_output("data:geometry:hybrid_powertrain:bop:length", units='m')
        self.add_output("data:geometry:hybrid_powertrain:bop:width", units='m')
        self.add_output("data:geometry:hybrid_powertrain:bop:height", units='m')
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        Considering ratios between the dimensions of the PowerCellution V Stack sized for 30 kW and the Power Generation
        System 30, we assume that for the whole FC system, the BoP : 
