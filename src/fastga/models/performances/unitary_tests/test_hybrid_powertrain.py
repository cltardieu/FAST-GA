...
from ..takeoff_HE import TakeOffPhase, _v2, _vr_from_v2, _v_lift_off_from_v2, _simulate_takeoff
...
def test_compute_cruise():
    ...
    assert fuel_mass == pytest.approx(7.57, abs=1)
    distance = (
...
def test_compute_descent():
    ...
    assert distance == pytest.approx(20, abs=1)
...
def test_loop_cruise_distance():
    ...
    assert m_total == pytest.approx(8.6, abs=1e-1)
